...
export const linePlugin: ComponentPlugin<LineComponent> = {
  ...
  render(comp, ctx: RenderContext): string {
    ...
    if (comp.stroke) {
      return wrapPlacement(comp, `#line(start: ${start}, end: ${end}, stroke: ${comp.stroke})`, ctx.offsetX, ctx.offsetY, ctx.flowMode);
    }
    ...
    return wrapPlacement(comp, `#line(start: ${start}, end: ${end}, stroke: ${stroke})`, ctx.offsetX, ctx.offsetY, ctx.flowMode);
  },
