...
function PaletteItem({ type, label, icon: Icon }: any) {
  ...
  useEffect(() => {
    ...
    const getTemplate = () => {
      ...
      switch (type) {
        case 'text': return { ...base, type: 'text', content: 'Double click to edit text', style: { fontSize: 10 }, width: 100, height: 10 };
        case 'table': return { ...base, type: 'table', dataSource: '{{items}}', columns: [], style: {}, showHeader: true, width: 180, height: 40 };
        case 'image': return { ...base, type: 'image', src: '/logo.png', width: 40, height: 40 };
        case 'line': return { ...base, type: 'line', thickness: '1pt', color: 'black', width: 180, height: 2 };
        case 'barcode': return { ...base, type: 'barcode', value: '{{code}}', format: 'code128', width: 40, height: 15 };
        case 'qr': return { ...base, type: 'qr', value: '{{url}}', format: 'qr', width: 25, height: 25 };
        case 'spacer': return { ...base, type: 'spacer', height: 10, width: 180 };
        default: return { ...base, type, width: 50, height: 20 };
      }
    };
