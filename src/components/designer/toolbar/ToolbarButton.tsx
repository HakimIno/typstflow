...
export const ToolbarButton = memo(function ToolbarButton({
  ...
  const variants = {
    default:
      'bg-[var(--bg-widget)] border-[var(--border-default)] text-[var(--text-primary)] hover:bg-[var(--bg-hover)] hover:border-[var(--border-accent)] shadow-sm',
    primary: 'bg-[var(--accent)] border-none text-white hover:brightness-110 shadow-sm',
    danger:
      'bg-[var(--color-danger-subtle)] border-[var(--color-danger-border)] text-[var(--color-danger)] hover:bg-[var(--color-danger-hover)]',
    ghost:
      'bg-transparent border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-black/5 dark:hover:bg-white/10',
    'toolbar-item':
  ...
  return (
    ...
    >
      ...
      {showChevron && (
        <svg
          className={clsx(
            'w-3 h-3 ml-0.5 transition-transform pointer-events-none',
            active && 'rotate-180'
          )}
          viewBox="0 0 24 24"
          fill="none"
