    ...
    <div className="p-2 space-y-4 bg-[var(--bg-widget)]">
      <section>
        ...
        <PropertyRow label="Repeat Header">
           <MiniInput 
             value={typeof component.repeatHeaderOnPage === 'boolean' ? String(component.repeatHeaderOnPage) : component.repeatHeaderOnPage}
             onChange={(v) => {
                let finalVal: any = v;
                if (v === 'true') finalVal = true;
                if (v === 'false') finalVal = false;
                updateComponent(component.id, { repeatHeaderOnPage: finalVal });
             }}
             placeholder="true / false / {{var}}"
