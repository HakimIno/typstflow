...
const sections: SheetNavSection[] = [
  ...
  { section: 'data', sectionKey: 'detailRows', rows: [row('d1', ['1', '2']), row('d2', ['3', '4'])] },
...
describe('table-nav', () => {
  ...
  it('stepCell moves across section boundaries and clamps at the edges', () => {
    ...
    expect(stepCell(flat.length, 2, { flatRow: 0, col: 0 }, -1, -1)).toEqual({ flatRow: 0, col: 0 });
  });
  ...
  it('sectionRange clamps the focus into the anchor section', () => {
    // Anchor in data (d1), focus pushed into the footer → clamped to d2
    const range = sectionRange(flat, 2, 'd1', 0, { flatRow: 3, col: 1 });
    expect(range?.rowIds).toEqual(['d1', 'd2']);
    expect(range?.cellIndices).toEqual([0, 1]);
    expect(range?.focus).toEqual({ flatRow: 2, col: 1 });
    expect(range?.section).toBe('data');
  });
});
...
describe('table-clipboard', () => {
  ...
  it('appends new rows via makeRow when pasting past the end of the band', () => {
    ...
    const out = applyPasteToRows(rows, 0, 0, [['a'], ['b'], ['c']], 2, (cols) =>
      row(`new-${n++}`, Array.from({ length: cols }, () => ''))
    );
